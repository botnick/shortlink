...
/** Stamp last_used_at, throttled to once a minute per key via KV. Opens its own
 *  DB handle — it runs in waitUntil, after the request's handle may be closed. */
export async function touchApiKey(
  env: AppBindings,
  keyId: string,
): Promise<void> {
  const guard = `apikeyts:${keyId}`;
  if ((await env.LINKS_KV.get(guard)) !== null) return;
  await env.LINKS_KV.put(guard, "1", { expirationTtl: 60 });
